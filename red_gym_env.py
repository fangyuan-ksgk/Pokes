...
class RedGymEnv(Env):
    ...
    def __init__(
        ...
        self.vec_dim = 4320 # = output_shape[0] * output_shape[1] * frame_stack
        ...
        self.col_steps = 16
        # 3 Frames stack together, why is there 2 memory padding? 
        self.output_full = (
    ...
    def reset(self, seed=None):
        ...
        # Q1: Here we load the initial state, I wonder where is the saved checkpoint stored at?
        ...
        # Q2: What does this do? It seems to relate to the statistics of the agent, but (memory_height, output_shape[1], 3) would make more sense?
    ...
    def init_knn(self):
        # Declaring index
        # vec_dim is the total dimensions of gray-scaled stacked frames (frame_stack * output_shape[0] * output_shape[1])
    ...
    def step(self, action):
        ...
        return obs_memory, new_reward*0.1, False, step_limit_reached, self.info
    ...
    def append_agent_stats(self, action):
        # This function is called after the 'run_action...' function, so actions is already taken, therefore is 'last_action'
    ...
    def update_frame_knn_index(self, frame_vec):
        # Q3: above 22 levels, we reset the knn index, why?
        if self.get_levels_sum() >= 22 and not self.levels_satisfied:
            self.levels_satisfied = True
            self.base_explore = self.knn_index.get_current_count()
            self.init_knn()

        # Unless there is no frames stored, we compare whether current frames is significantly different from the nearest frame,
        # and only add current frame if it is different enough
        # Q4: What if the number of stroed frames exceeds the limit? well the num_frames is set to 20000, which is huge
        if self.knn_index.get_current_count() == 0:
            ...
            # distances[0][0]: the minimal squared l2 distance between current frame and queried frame
            # similar_frame_dist: the threshold for considering two frames are similar
            if distances[0][0] > self.similar_frame_dist:
                # print(f"distances[0][0] : {distances[0][0]} similar_frame_dist : {self.similar_frame_dist}")
                self.knn_index.add_items(
                    frame_vec, np.array([self.knn_index.get_current_count()])
                )
    # This is the first attempt of exploration: Just record the coordinates of the agent
    # I guess the obvious issue is that the agent does not proceed as fast, and level up is not considered
    ...
    def update_reward(self):
        # compute reward
        # Q5: why is the group_rewards function different than the total_reward function?
        # -- group_rewards is used for memory, total_reward is used for the agent
        ...
        # This reward is a static value, not incremental
    ...
    def create_exploration_memory(self):
        ...
        def make_reward_channel(r_val):
            ...
            col = col % h
            row = row % w
            # print('Make reward channel part explained: ')
            # print(f'-- col_steps {col_steps} |  max_r_val {max_r_val} | row {row} | row_covered {row_covered} | memory shape {memory.shape}')
            # print(f'-- col {col}')
            memory[:col, row] = 255
    ...
    def get_game_state_reward(self, print_stats=False):
        ...
        # This seems to be the experience of each pokemon, why not use it?
    ...
    def get_pokemon_exp(self):
        return [self.read_triple(a) for a in [0xD179, 0xD1A5, 0xD1D1, 0xD1FD, 0xD229, 0xD255]]
    def get_pokemon_levels(self):
        return [self.read_m(a) for a in [0xD18C, 0xD1B8, 0xD1E4, 0xD210, 0xD23C, 0xD268]]

    # Function is useful for inspecting the game state
    ...
    def read_hp_fractions(self):
        curr_hp_loc = [0xD16C, 0xD198, 0xD1C4, 0xD1F0, 0xD21C, 0xD248]
        max_hp_loc = [0xD18D, 0xD1B9, 0xD1E5, 0xD211, 0xD23D, 0xD269]
        return [self.read_hp(curr_hp_loc[i]) / self.read_hp(max_hp_loc[i]) if self.read_hp(max_hp_loc[i]) != 0 else 0 for i in range(6)]

    def read_hp(self, start):
        return 256 * self.read_m(start) + self.read_m(start+1)
    
    def read_double_m(self, start):
        return 256 * self.read_m(start) + self.read_m(start+1)
