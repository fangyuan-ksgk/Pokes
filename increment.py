...
class GroundEnv(RedGymEnv):
    def __init__(self, env_config):
        ...
        self.context = 3
        self.mode_dict = {'trainer': 0, 'healer': 1, 'explorer': 2}
        self.mode = 'healer'
        self.done_count = 0
    ...
    def check_if_done(self):
        ...
        if self.mode == 'healer':
            self.done_count += int(self.info['status']['death_count'] > 0)
            done = done or (self.done_count>2) # allow few terms before terminate from death
            # done = done or self.info['status']['death_count'] > 0
            # if done:
                # print('\n Died')
                # print('\n ------------------------------------- \n')
        return done
    ...
    # Overwrite the exploration reward function
    def update_frame_knn_index(self, frame_vec):
        # Update frame knn only when not in battle
        if self.context != self.context_dict['in_battle']:
            return super().update_frame_knn_index(frame_vec)
